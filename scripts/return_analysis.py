...
st.set_page_config(
    page_title="Prediction Returns Analysis",
    page_icon="📈",
    layout="wide",
...
def calculate_prediction_returns(prediction_data: pd.DataFrame, window_size: int = 20) -> Dict:
    """
    Calculate returns for each prediction type over the specified window.
    
    Parameters:
    - prediction_data: DataFrame with stock data and predictions
    ...
    for i in range(len(prediction_data) - window_size):
        ...
        # Get the predicted label for this point
        label = prediction_data['predicted_label'].iloc[i]
...
def plot_returns_histogram(returns_data: pd.DataFrame, label: int, label_name: str) -> go.Figure:
    """Create a histogram of returns for a given prediction label."""
    ...
    # Update layout
    fig.update_layout(
        title=f"Distribution of Returns for '{label_name}' Predictions",
        xaxis_title="Return",
        yaxis_title="Count",
        showlegend=False,
        xaxis_tickformat='.1%'
...
def plot_returns_over_time(returns_data: pd.DataFrame, label: int, label_name: str) -> go.Figure:
    """Create a line chart of returns over time for a given prediction label."""
    # Sort data by date
    ...
    # Update layout
    fig.update_layout(
        title=f"Returns Over Time for '{label_name}' Predictions",
        xaxis_title="Date",
        yaxis_title="Return",
        showlegend=False,
        yaxis_tickformat='.1%'
...
def plot_returns_vs_probability(returns_data: pd.DataFrame, label: int, label_name: str) -> go.Figure:
    ...
    # Update layout
    fig.update_layout(
        title=f"Returns vs. Prediction Probability for '{label_name}' Predictions",
...
def plot_cumulative_returns(returns_data: pd.DataFrame, label: int, label_name: str) -> go.Figure:
    """Create a chart showing cumulative returns if following predictions of this type."""
    # Sort by date
    ...
    # Update layout
    fig.update_layout(
        title=f"Cumulative Returns Following '{label_name}' Predictions",
        xaxis_title="Date",
...
def main():
    try:
        ...
        with DBSession() as session:
            # Page title
            st.title("Prediction Returns Analysis")
            st.write("Analyze the returns achieved by following model predictions")
            ...
            with st.sidebar:
                ...
                st.header("Legend")
                st.markdown("🟢 Up (0): Predicted uptrend")
                st.markdown("⚫ Side (1): Predicted sideways movement")
                st.markdown("🔴 Down (2): Predicted downtrend")
            ...
            with st.spinner("Calculating returns..."):
                returns_results = calculate_prediction_returns(prediction_data, window_size)
            ...
            st.header("Detailed Analysis by Prediction Type")
            ...
            if tab_labels:
                ...
                for i, label in enumerate(tab_labels):
                    with tabs[i]:
                        ...
                        st.write(f"Analysis of returns when the model predicted '{label_names[label]}'")
