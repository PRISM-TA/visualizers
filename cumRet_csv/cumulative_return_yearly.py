...
def plot_multiple_stocks(stock_files, output_filepath):
    ...
    for i, stock_file in enumerate(stock_files):
        ...
        yearly_cumulative_return = data.groupby('year')['benchmark_cum_return_pct'].last()

        # Plot the cumulative return on the respective subplot
        ax = axes[i]
        ax.plot(yearly_cumulative_return.index, yearly_cumulative_return.values, marker='o')
        ax.set_title(stock_file.split('/')[-1].replace('.csv', ''))  # Use the filename as the title
        ax.set_xlabel('Year')
        ax.set_ylabel('Cumulative Return (%)')
        ax.set_xticks(yearly_cumulative_return.index[::2])  # Show every other year to reduce overlap
        ax.grid(True)
